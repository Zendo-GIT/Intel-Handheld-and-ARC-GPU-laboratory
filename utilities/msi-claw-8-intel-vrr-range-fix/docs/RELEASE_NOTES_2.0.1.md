# MSI Claw Intel VRR Range Fix 2.0.1

Release 2.0.1 extends the reversible Intel VRR package introduced in 2.0.0 by
integrating the shared LFC x2 correction into every managed profile. It remains
restricted to the exact `CSW0801 / PN8007QB1-2` panel used by validated MSI
Claw 8 AI+ and Claw 8 EX AI+ configurations.
...
The same shared correction is installed by every 2.0.1 profile. Each path
requires the exact managed mode, EDID and requested Intel range before changing
the two solution flags, and verifies both flags afterward. The x2 removal itself
has been observed and validated on real hardware at 30-120 Hz; integration in
48-120 and both 144 Hz trials does not by itself claim separate hardware-result
validation. In a 48 Hz-floor profile, Intel refresh multiplication below 48 FPS
is unavailable while the correction is active.

## Four profiles

| Installer | Role | Shared LFC correction | Validation boundary |
|---|---|---|---|
| `INSTALL_30_120_VRR.bat` | Default 2.0.1 profile | Included; x2 removal hardware-validated | Corrected combination validated on the reference Arc 140V system |
| `INSTALL_48_120_VRR.bat` | Official Intel/MSI profile | Included with exact 48-120 readback | Native panel range through Intel's public `EXCELLENT` profile |
| `INSTALL_EXPERIMENTAL_48_144_VRR.bat` | Guarded panel-overclock trial | Included and required before confirmation | Fixed 144 Hz was stable on tested units; VRR at 144 Hz is not guaranteed |
| `INSTALL_EXPERIMENTAL_30_144_VRR.bat` | Guarded full-range trial | Included and required before confirmation | Visibly flickered on the reference panel; not recommended |

## 144 Hz failsafe

The two 144 Hz installers do not silently keep an unconfirmed range. At the next
sign-in, the tool waits until the driver verifies the requested profile and both
shared LFC flags, observes the complete state for 20 seconds, and displays a
system-modal Yes/No confirmation.
...
- Failure to verify the requested 144 Hz range or shared LFC flags also triggers
  complete LFC/VRR rollback.
