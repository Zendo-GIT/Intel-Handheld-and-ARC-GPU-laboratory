...
try {
    switch ($Action) {
        'Schedule' {
            ...
            foreach ($path in @($sourceLauncherPath, $sourceDriverInterfacePath, $installedMainToolPath)) {
                if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        ...
        'Confirm' {
            ...
            for ($attempt = 1; $attempt -le 90; $attempt++) {
                try {
                    ...
                    if ($driverState.Supported -and $driverState.Enabled -and
                        ...
                        [int]$driverState.MaximumHz -eq 144) {
                        $verified = $true
                        break
                    }
                }
            ...
            if (-not $verified) {
                Restore-ExperimentalProfile -Reason 'the driver did not verify the requested range after sign-in'
                break
            }

            Start-Sleep -Seconds $observationSeconds
            $rangeText = "$expectedMinimumHz-144 Hz"
            $message = "The experimental $rangeText profile has been active and verified for 20 seconds.`r`n`r`nKeep this custom VRR range?`r`n`r`nYES: keep it and remove the trial task.`r`nNO or no answer: restore the previous profile and restart Windows.`r`n`r`nThe test will default to NO after 30 seconds."
